...
class MetroMap extends React.Component {
  ...
  updateScale() {
    const scale =
      this.props.width / this.aspectRatio < this.props.height
    ...
    this.scaledStations = this.metroData.stations.map((station, i) => ({
      ...
      center: {
        x: station.center.x * this.scale,
        y: station.center.y * this.scale,
      },
      lines: station.lines,
    }));
  }
  ...
  render() {
    ...
    const stations = this.metroData.stations.map((station, i) => (
      <MetroStation
        key={i}
        cx={station.center.x * this.scale}
        cy={station.center.y * this.scale}
        ...
        onMouseEnterStation={() =>
          this.props.onMouseEnterStation(this.scaledStations[i], this.scale)
        }
        onMouseLeaveStation={() =>
          this.props.onMouseLeaveStation(this.scaledStations[i], this.scale)
        }
        onClickStation={e =>
          this.props.onClickStation(this.scaledStations[i], this.scale, e)
        }
      />
    ));
    ...
    const connections = this.metroData.connections.map((connection, i) => (
      <path
        ...
        d={connection.path.reduce((acc, cmd) => {
          const coordinates = cmd.coordinates
            .map(x => x * this.scale)
            .join(',');
    ...
    if (this.props.showStationName) {
      stationNames = this.metroData.stations.map((station, i) => (
        <text
          key={i}
          x={station.name.pos.x * this.scale}
          y={station.name.pos.y * this.scale}
          style={this.props.textStyle}
          dominantBaseline="middle"
          textAnchor={station.name.pos.anchor}>
          {station.name.zh}
        </text>
      ));
    }
    ...
    if (this.props.showUserData) {
      if (typeof this.props.renderUserData === 'function') {
        userData = (
          <g>
            {this.scaledStations.map(station =>
              this.props.renderUserData(station, this.scale),
            )}
          </g>
        );
        ...
        userData = this.props.renderUserData.map((fn, i) => (
          <g key={i}>
            {this.scaledStations.map(station => fn(station, this.scale))}
          </g>
        ));
      }
    }
    return (
      <svg style={svgStyle}>
        <g fill={this.props.bgColor} strokeWidth={4 * this.scale}>
          {connections}
        </g>
        <g>{stations}</g>
        {userData}
